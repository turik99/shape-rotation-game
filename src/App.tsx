...
function App() {
  ...
  //State variables describing win conditions, styles, camera positions, user choices.
  ...
  //Creates and updates the shapes on screen after every round.
  ...
  //Updates UI and handles recycling data when the win condition is reached.
  ...
  useEffect(() => {
    //Effect to gather new camera info whenever the user checks the shapes.
    updateCameraInfo(true)
  }, [checkCount])

  //Show the intro screen if the user hasn't started yet.
  if (showIntro) {
  ...
  //Show the ending page if the user has completed their game.
  if (gamesPlayed === 3) {
  ...
  //Describes the gameplay components and styles.
  else {
        ...
        <Container fluid>
          ...
          <Row style={{ marginTop: 0 }}>
            ...
            <Col style={{ marginLeft: "-24pt" }}>
              <div>
                <CustomConfetti show={showConfetti.show} colors={showConfetti.colors} />
...
function CustomConfetti(props: CustomConfettiProps) {
